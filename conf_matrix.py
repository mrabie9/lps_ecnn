...
datasets = {
    ...
    "Radar_noisy"    : {"models" : "radar/exp-radar-elc-noisy-radhcar/", "tasks" : "radar/tasks-noisy-radchar/", "num_classes":12, "offset": [0,6,11]}
}
args = args_class("")
    ...
    if eval_snr:
        ...
        for snr in snr_range:
            ...
            if os.path.exists(save_dir) and not recollect_data:
                all_labels, all_preds, unique_labels, omega, pred_class, util, inv_util = load_test_outputs(save_dir)
                ...
                save_test_outputs(os.path.join(model_path, task), all_labels, all_preds, unique_labels, omega, pred_class, util, inv_util, filename=save_dir)
            
            all_correct.extend(correct)
            all_eu.extend(inv_util/100)
            continue
            ...
            best = choose_tau(sweep_results, target_coverage=0.90)  # accept ~80% most-certain
            sel_acc = best["sel_acc"]*100
            cov = best['coverage']
        ...
        # with open(f"../LPSforECNN/eu_comparison/ELC/distrib/{task}_snr_results_selective_noisy.pkl", "wb") as handle:
        with open(f"../LPSforECNN/eu_comparison/ELC/distrib/{task}_snr_results_distrib.pkl", "wb") as handle:
            pickle.dump(payload, handle)
        ...
        with open(f"../LPSforECNN/eu_comparison/ELC/coverage/{task}_coverage_data_noisy.pkl", "wb") as handle:
            pickle.dump(payload, handle)
        # plot_multiclass_roc_auc(all_labels, 
        #                             np.eye(len(unique_labels))[all_preds],
        #                             class_names=[str(lbl) for lbl in unique_labels],
