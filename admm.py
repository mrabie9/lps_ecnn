    ...
    def init(self, args, config, model):
        ...
        for (name, W) in model.named_parameters():
            if name in self.prune_ratios:
                ...
                self.ADMM_Z[name] = W.detach().clone()
                self.ADMM_U[name] = torch.zeros_like(W)
                ...
                self.ADMM_Y[name] = W.detach().clone()
                self.ADMM_K[name] = torch.zeros_like(W)


                # print("Y init", torch.max(self.ADMM_Y[name]), torch.min(self.ADMM_Y[name]))
            # if(len(W.size()) == 4):
...
def mask_pruning(ratio, submask, mask):
    ...
    non_zero_elements = mask_temp[mask_temp != 0]

    if non_zero_elements.size == 0:
        raise ValueError("Empty mask after filtering — cannot compute percentile")
...
def z_u_update(args, ADMM, model, device, train_loader, optimizer, epoch, data, batch_idx, writer):
    ...
    if epoch != 1 and (epoch - 1) % args.admm_epochs == 0 and batch_idx == 0:
        for i, (name, W) in enumerate(model.named_parameters()):
            ...
            check_nan(W, f'W_{name}')
            check_nan(ADMM.ADMM_U[name], f'U_{name}')
            check_nan(ADMM.ADMM_Z[name], f'Z_{name}')
...
def y_k_update(args, ADMM, model, device, train_loader, optimizer, epoch, data, batch_idx, writer):
    ...
    if epoch != 1 and (epoch - 1) % args.mask_admm_epochs == 0 and batch_idx == 0:
        for i, (name, M) in enumerate(model.named_parameters()):
            ...
            check_nan(M, f'M_{name}')
            check_nan(ADMM.ADMM_Y[name], f'Y_{name}')
            check_nan(ADMM.ADMM_K[name], f'K_{name}')
...
def append_admm_loss(args, ADMM, model, ce_loss):
    ...
    for i, (name, W) in enumerate(model.named_parameters()):  ## initialize Z (for both weights and bias)
        if name in ADMM.prune_ratios:
            ...
            diff = W - ADMM.ADMM_Z[name] + ADMM.ADMM_U[name]
            diff = torch.clamp(diff, min=-1, max=1)
            # print(diff)
            # print("W:", torch.max(W), torch.min(W))
            # print("ADMM.ADMM_Y[name]:", torch.max(ADMM.ADMM_Y[name]), torch.min(ADMM.ADMM_Y[name]))
            # print("ADMM.ADMM_K[name]:", torch.max(ADMM.ADMM_K[name]), torch.min(ADMM.ADMM_K[name]))
            # print("diff:", torch.max(diff), torch.min(diff))

            if torch.isnan(diff).any() or torch.isinf(diff).any():
                raise ValueError(f"Z - NaN or Inf in diff term for {name}")

            norm_sq = torch.sum(diff**2)
            
            if torch.isnan(norm_sq) or torch.isinf(norm_sq):
                print("diff:",torch.max(diff), torch.min(diff))
                raise ValueError(f"Z - Invalid norm_sq for {name}: {norm_sq.item()}")

            rho = ADMM.rhos[name]
            if isinstance(rho, torch.Tensor) and (torch.isnan(rho) or torch.isinf(rho)):
                raise ValueError(f"Z - Invalid rho for {name}: {rho.item()}")

            # admm_loss[name] = 0.5 * rho * norm_sq
            admm_val = 0.5 * rho * norm_sq
            if torch.isnan(admm_val) or torch.isinf(admm_val):
                print(f"[{name}] admm loss term is invalid: {admm_val.item()}")
                print(f"norm_sq = {norm_sq.item()}, rho = {rho}")
                raise ValueError(f"Computed NaN ADMM loss in {name}")
            admm_loss[name] = admm_val
            
    mixed_loss = 0
    mixed_loss += ce_loss
    for k, v in admm_loss.items():
        if v.item() > 1e6:
            print(f"[{k}] HUGE ADMM term: {v.item()}")
        prev_loss = mixed_loss.clone()
        mixed_loss += v
        if torch.isnan(mixed_loss).any():
            raise ValueError(f"Mixed loss is NaN: {prev_loss} + {v}")
...
def append_mask_loss(args, ADMM, model, ce_loss):
    ...
    for i, (name, W) in enumerate(model.named_parameters()):  ## initialize Z (for both weights and bias)
        # for tensor_name, tensor in [('W', W)]:
        #     print(f"NaN in {tensor_name} for {name}")
        ...
        # #         if torch.isnan(tensor).any():
        # #             print(f"NaN in {tensor_name} for {name}")
        #     value1 = 0.5 * ADMM.rhos_mask[name] 
        #     value2 = W - ADMM.ADMM_Y[name] 
        #     value3 = ADMM.ADMM_K[name]
        #     value4 = torch.norm(value2 + value3, p=2)**2
        #     admm_loss[name] = 0.5 * ADMM.rhos_mask[name] * (torch.norm(W - ADMM.ADMM_Y[name] + ADMM.ADMM_K[name], p=2) ** 2)

        if 'w_mask' in name:
            diff = W - ADMM.ADMM_Y[name] + ADMM.ADMM_K[name]
            diff = torch.clamp(diff, min=-3, max=3)
            # print("W:", torch.max(W), torch.min(W))
            # print("ADMM.ADMM_Y[name]:", torch.max(ADMM.ADMM_Y[name]), torch.min(ADMM.ADMM_Y[name]))
            # print("ADMM.ADMM_K[name]:", torch.max(ADMM.ADMM_K[name]), torch.min(ADMM.ADMM_K[name]))
            # print("diff:", torch.max(diff), torch.min(diff))
            ...
            if torch.isnan(diff).any() or torch.isinf(diff).any():
                raise ValueError(f"NaN or Inf in diff term for {name}")

            norm_sq = torch.norm(diff, p=2) ** 2
            # print("norm_sq:",torch.max(norm_sq), torch.min(norm_sq))
            if torch.isnan(norm_sq) or torch.isinf(norm_sq):
                raise ValueError(f"Invalid norm_sq for {name}: {norm_sq.item()}")

            rho = ADMM.rhos_mask[name]
            if isinstance(rho, torch.Tensor) and (torch.isnan(rho) or torch.isinf(rho)):
                raise ValueError(f"Invalid rho for {name}: {rho.item()}")
